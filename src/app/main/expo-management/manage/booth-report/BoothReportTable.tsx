...
import { getBoothReportAPI } from "./apis/Get-booth-Api";
import { useAttendeesSelector } from "./BoothReportSlice";
...
function BoothTable({
  ...
  const defaultColumns = [

    {
      ...
      header: t("booth_name"),
      id: "boothName",
      Cell: ({ row }) => <Typography>{row.original.boDetails.details.boothName}</Typography>,
    },
    {
      accessorKey: "boothManagerName",
      header: t("manager_name"),
      id: "boothManagerName",
      Cell: ({ row }) => <Typography>{row.original.boDetails.details.boothManager.name}</Typography>,
    },

    {
      accessorKey: "boothManagerEmail",
      header: t("manager_email"),
      id: "boothManagerEmail",
      Cell: ({ row }) => <Typography>{row.original.boDetails.details.boothManager.email}</Typography>,
    },
    {
      ...
      Cell:  ({ row }) => <Typography> {row.original.boCreatedAt}</Typography>,
    },
    {
      accessorKey: "userName",  
      header: t("attendee_name"),
      id: "userName",
      Cell: ({ row }) => <Typography>{row.original.boDetails.details.name}</Typography>,
    },
    {
      accessorKey: "userEmail",
      header: t("attendee_email"),
      id: "userEmail",
      Cell: ({ row }) => <Typography>{row.original.boDetails.details.email}</Typography>,
    },
  ];
    ...
    (async () => {
      try {
        ...
        const dataTableLocalConfig = await LocalCache.getItem(
          `dataTableLocalConfigBoothReport_${user.uuid}`
        );
  ...
  const getAttendeesData = useCallback(
    debounce(async ({ pagination, keyword, sorting, expoDetails }) => {
      ...
      try {
        ...
        const response = await getBoothReportAPI({
          pagination,
          keyword,
          sorting,
          expoDetail,
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", "booth_report.xlsx");
