...
import store, { AppThunk, RootState } from 'app/store/store';
...
const langUpdateHandler = () => {
	store.dispatch(updateNavigationItem(navigationConfig[0].id, { timestamp: Date.now() }))
}

window.removeEventListener("lang-updated", langUpdateHandler);
window.addEventListener("lang-updated", langUpdateHandler);
